...
def plot_values(files):
    plot_amount = int(len(files)/len(objects))
    diff = 200
...
def plot(file_list):
    ...
    for index_folder, files in enumerate(file_list):
        ...
        amount_per_category = int(len(files)/len(objects))

        for index_file in range(amount_per_category):        
            categorized = list_categorization(files)
            title = subplot_title([os.path.basename(categorized[i][index_file]) for i in range(len(objects))])
