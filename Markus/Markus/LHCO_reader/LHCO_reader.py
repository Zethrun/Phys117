...
NAMES_DICT = dict(list(zip(NUMBERS, NAMES)))
...
class PrintDict(OrderedDict):
    ...
    def __str__(self):
        ...
        table = pt(list(self.keys()))
        table.add_row(list(self.values()))
...
class Event(dict):
    ...
    def number(self, anti_lepton=False):
        ...
        for name, objects in self.items():
    ...
    def count_parsed(self):
        ...
        9
        """
        return sum([len(objects) for objects in self.values()])
    ...
    def __parse(self):
        ...
        for line in self._lines:
            ...
            if number == "0":  # "0" events are trigger information
                assert len(words) == 3, "Trigger should have 3 words"
                self.trigger_info = list(map(int, words[1:]))
            else:
                ...
                self.add_object(name, list(zip(PROPERTIES, values)))
    ...
    def __add__(self, other):
        ...
        """
        combination = Event()
        for name in list(self.keys()):  # e.g. "electron"
    ...
    def multiplicity(self):
        ...
        numbers = [self.number()[name] for name in NAMES if name != "MET"]
        return sum(numbers)
    ...
    def ET(self):
        ...
        ET = 0.
        no_MET_names = (name for name in NAMES if name != "MET")
